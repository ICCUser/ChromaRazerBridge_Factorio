...
Factorio), avec rechargement a chaud depuis DEUX sources :
  ...
  2. script-output/chroma_mapping.json           -> overrides ecrits par
     l'interface en jeu (menu Chroma Bridge, CONTROL+SHIFT+C)
...
from factorio_watcher import default_script_output_dir

DEFAULT_MAPPING_PATH = Path(__file__).parent / "mapping.json"
INGAME_OVERRIDES_PATH = default_script_output_dir() / "chroma_mapping.json"

DEFAULTS = {
    "layout": "azerty_fr",
    "poll_seconds": 0.2,
    "ambient": {"devices": ["mouse", "chromalink"], "color1": [230, 100, 20], "color2": [20, 10, 0], "thresholds": []},
    "keyboard_idle": {"color1": [230, 100, 20], "color2": [20, 10, 0]},
    "research_bar": {
        "enabled": True,
...
class MappingStore:
    ...
    def __init__(self, path: Path = DEFAULT_MAPPING_PATH, overrides_path: Path = INGAME_OVERRIDES_PATH):
        self.path = path
    ...
    def reload(self, force: bool = False) -> bool:
        ...
        overrides_mtime = self._mtime_of(self.overrides_path)
        ...
        overrides = _read_json(self.overrides_path)
        if overrides:
